# %%
#Converts tracking and event buli data to statsbomb - legacy code
import xml.etree.ElementTree as ET
import pandas as pd
import numpy as np
#shouldn't there be flipping?
def convert_event_and_tracking(input_match_id, players_path, tracking_path, together_csv_path, event_path, matchplan_path, event_outputpath, tracking_outputpath):
    ...
    # %%
    players = load_players(players_path, True)
    ...
    # %%
    together = pd.read_csv(together_csv_path,sep = ';', encoding='latin-1', on_bad_lines='skip')
    together = pd.concat([
    together[~together.duplicated(subset='FRAME_NUMBER', keep=False)],  # Keep non-duplicates
    together[together['XRec'].notna()]             # Keep duplicates where 'xrec' is not NaN - also need to add case when only one pass exists
    ])
    ...
    tracking_together = pd.merge(tracking, together, left_on = "N", right_on = "FRAME_NUMBER")[["EVENT_ID", "TeamId", "PersonId", "N", "X", "Y", "CUID1", "PUID1"]]
    ...
    # %%
    matchplan = pd.read_xml(matchplan_path, xpath = ".//Fixture")
    ...
    while event[event["possession_team"].isna()].shape[0] != 0:
        #print(event[event["possession_team"].isna()].shape[0])
    ...
    event["team"] = [poss["id"] for poss in event["possession_team"]]
    ...
    playernames = load_players(players, False)
    ...
    for i in needs.index:#finding closest is not perfect - receiver can carry - first time when below a certain threshold??
        ...
        rel_df["EVENT_ID"] = eventid#also dumb way to do this but it works
    ...
    # %%
    all_unsuccess[all_unsuccess["EVENT_ID"] == 18453000000723]
    ...
    together["YRec"] = 80 - (1.09361 * -1 * together["YRec"].astype(float) + 40)
    ...
    # %%
    event[event["EVENT_ID_x"] == 18453000000723][["end_location", "location"]]
    ...
    outcomes = {"BlockedShot": {"id": 96, "name":"Blocked"}, "ShotWide":{"id":98, "name":"Off T"},
                ...
                "SuccessfulShot":{"id":97, "name":"Goal"}}
    ...
    def replace_shot_end_location(row):
        if row['SUBTYPE'] in ["BlocketShot", "ShotWide", "ShotWoodWork", "SavedShot"]:
    ...
    # %%
    s11 = load_players(players_path, False)
    ...
    # %%
    converted_s11["type"] = np.where(converted_s11["pass"].notna(), {'id': 30, 'name': 'Pass'}, converted_s11["type"])
    converted_s11["type"] = np.where(converted_s11["shot"].notna(), {'id': 16, 'name': 'Shot'}, converted_s11["type"])
    ...
    convert_to_json(converted_s11, event_outputpath)
    convert_to_json(tracking_converted, tracking_outputpath)
