...
class UnxpassComponent(ABC):
    ...
    def initialize_dataset(self, dataset: Union[PassesDataset, Callable]) -> PassesDataset:
        ...
        return dataset
    def initialize_dataset_2(self, dataset: Union[PassesDataset, Callable], nums) -> PassesDataset:
        if callable(dataset):
            return dataset(xfns=self.features.head(nums), yfns=self.label.head(nums), transform=self.transform)
        return dataset
    @abstractmethod
...
class UnxPassPytorchComponent(UnxpassComponent):
    ...
    def new_predict(self, data, batch_size=1, num_workers=0, pin_memory=False) -> pd.Series:
        # Load dataset
        dataloader = DataLoader(
            data,
            shuffle=False,
