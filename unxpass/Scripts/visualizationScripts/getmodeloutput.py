...
model_pass_wSpeeds = pass_value_speeds.SoccerMapComponent(
    model=mlflow.pytorch.load_model(
        'runs:/5b9c622dd1d246f4aabc5de562b6e9d2/model', map_location = 'cpu'
...
pdf_filename = "value_def_fail_test_player_notrain.pdf"
...
with PdfPages(pdf_filename) as pdf:
    for idx, row in tqdm(sequences.iloc[0:20].iterrows()):
        id = row['id']
        ...
        title = f"Game {game_id}, statsbomb id {id}, index {action_id}, max_val {np.amax(surface)}"
        
        
        # 
        fig = plotPlays.visualize_surface(ff_action, start_action, (game_id, action_id), ball_speed = speed_action, title = title, surface = surface, surface_kwargs= surface_type, log = False, modelType = "val")
