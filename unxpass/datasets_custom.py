...
class PassesDataset(Dataset):
    ...
    def limit_dataset(self, n: int):
        """Limit the dataset to the first n passes.

        Parameters
        ----------
        n : int
            The number of passes to keep in the dataset.
        """
        if self._features is not None:
            self._features = self._features.iloc[:n]
        if self._labels is not None:
            self._labels = self._labels.iloc[:n]
    @property
    def features(self):
    ...
    @features.setter
    def features(self, new_df):
        self._features = new_df
...
class CompletedPassesDataset(PassesDataset):
    ...
    def __init__(
        ...
        super().__init__(xfns, yfns, transform, path)
