#converts hawkeye data to statsbomb - legacy code
import pandas as pd
import numpy as np
...
import orjson
def read_Hawkeye_player_loc(file_path, period, minute, second_range, team,actor, action_id, player_to_team, goalkeepers, uefa_map):
    """
    ...
    player_df = player_df[(player_df['time'] >= second) & (player_df['time'] <= max_second)]#can be empty if dealing with overflows
    player_df = player_df.sort_values(by = ['time'])
    #print(file_path)
    if player_df.shape[0] == 0:
        print("Empty DataFrame...")
        return pd.DataFrame(columns = ['event_uuid', 'visible_area', 'freeze_frame'])
    player_df['minute'] = minute
    #player_df['pos'] = player_df.apply(lambda d: d['centroid'][0]['pos'], axis = 1)
    player_df['pos'] = [centroid[0]['pos'] for centroid in player_df['centroid']]
    #player_df['pos_name'] = player_df.apply(lambda d: d['role']['name'], axis = 1)
    player_df['pos_name'] = [role['name'] for role in player_df['role']]
    player_df = player_df[(player_df['pos_name'] == "Goalkeeper") | (player_df['pos_name'] == "Outfielder")]
    #print(player_df['personId'].iloc[0])
    #print(player_df)
    #print(player_df.apply(lambda d: d['personId']['uefaId'], axis = 1))
    player_df = player_df[player_df['personId'] != {}] #ensure that uefaId exists? weird edge case
